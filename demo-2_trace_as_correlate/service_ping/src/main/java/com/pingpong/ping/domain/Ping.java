...
public final class Ping {
    ...
    /**
     * Action #1 (mutation): create a new ping and raise {@link PingCreated}. The {@code faulty}
     * flag rides the same event so the downstream failure scenario uses the identical flow.
     */
    public static Ping create(String id, String note, boolean faulty) {
        Ping ping = new Ping(id);
        ping.note = note;
        ping.status = "CREATED";
        ping.pending_events.add(new PingCreated(id, note, faulty, Instant.now()));
        return ping;
    }
